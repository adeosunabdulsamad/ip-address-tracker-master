...
let ipPostalCode = document.getElementById("ip-postalCode");
...
function myFunction() {
    ...
    fetch(currentIpifyUrl)
        ...
        .then(data => {
            ...
            ipLocation.textContent = data.location ? `${data.location.country}, ${data.location.region}` : 'N/A';
            ipTimezone.textContent = data.location ? `UTC ${data.location.timezone}` : 'N/A';
            ipISP.textContent = data.isp || 'N/A';
            ipPostalCode.textContent = data.location ? data.location.postalCode : 'N/A';
